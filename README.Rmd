...
A `heuristika` model can be fit against the series by additionally specifying the expected period length `m` of the seasonal component.
...
We don't import `ggplot2` by default, but if it is made available one can use its `autoplot` generic to visualize the fitted model as `heuristika` provides methods for its classes.
...
```{r predict_heuristika}
forecast <- predict(object = fitted_model, h = 12, n = 10000)
```

The returned `forecast` object has it's own `heuristika_paths` class which can again be plotted using `autoplot()`:

```{r autoplot_forecast_forecast}
autoplot(forecast, date = dates, date_future = dates_future)
```

Special about `heuristika` is that sample paths from the joint forecast distribution are the native output of `heuristika`s predict method--in contrast to, for example, the usual point forecasts or pre-aggregated quantiles.
