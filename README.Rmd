...
This graph hides, however, that sample paths from the joint forecast distribution are the native output of `tulip`s `predict` method---in contrast to the usual point forecasts or pre-aggregated quantiles.
...
Forecasts from state space models [can be susceptible to anomalous observations](https://minimizeregret.com/post/2022/07/05/state-space-model-intricacies/). Especially when more recent observations are surprisingly different from previous observations, state space models can pick up large trends. In reality, however, those trends do not realize, leading to very poor forecasts.
...
```{r resex_data, message = FALSE, warning = FALSE}
library(RobStatTM)
...
```{r resex_structts_plot, echo = FALSE}
...
```{r intermittent_data_fit}
fitted_model <- tulip::tulip(y = y, m = 12)
fitted_model$family <- "bootstrap"
```

```{r intermittent_data_forecast}
forecast <- predict(
  object = fitted_model,
  h = 12,
  n = 10000,
...
```{r intermittent_data_plot_paths}
autoplot(forecast, method = "paths")
```
...
Sarah Gelper, Roland Fried, Cristophe Croux (2007). *Robust Forecasting with Exponential and Holt-Winters Smoothing*. https://ssrn.com/abstract=1089493
